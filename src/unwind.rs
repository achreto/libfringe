...
pub unsafe extern "C" fn unwind_wrapper(
  arg: usize,
  sp: StackPointer,
  stack_base: *mut u8,
  f: unsafe extern "C" fn(usize, StackPointer),
) -> Option<Box<Box<dyn Any + Send>>> {
...
// exception object.
pub unsafe extern "C" fn start_unwind(panic: Box<Box<dyn Any + Send>>) -> ! {
...
pub fn unwind_arg(stack_base: *mut u8) -> usize {
  let marker = UnwindMarker { stack_base };
  Box::into_raw(Box::new(Box::new(marker) as Box<dyn Any + Send>)) as usize
}
